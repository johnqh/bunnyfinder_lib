...
Business logic library with Zustand stores for the BunnyFinder application.

**npm**: `@sudobility/entitybunnyfinder_lib` (restricted, BUSL-1.1)
...
Unified hook that combines entitybunnyfinder_client hooks + Zustand store + business logic:
...
This is the primary hook consumed by UI layers (entitybunnyfinder_app, entityentitybunnyfinder_app_rn).
...
```
entitybunnyfinder_app / entityentitybunnyfinder_app_rn
    ↓ uses
@sudobility/entitybunnyfinder_lib (this package)
    ↓ uses
@sudobility/entitybunnyfinder_client (API hooks)
    ↓ uses
@sudobility/entitybunnyfinder_types (type definitions)
```

## Related Projects

- **entitybunnyfinder_types** — Shared type definitions; imported transitively via entitybunnyfinder_client
- **entitybunnyfinder_client** — API client SDK; this library wraps its hooks with business logic and Zustand state
- **entitybunnyfinder_app** — Web frontend that consumes `useHistoriesManager` from this library
- **entityentitybunnyfinder_app_rn** — React Native app that consumes `useHistoriesManager` via file: links
- **entitybunnyfinder_api** — Backend server; this library communicates with it indirectly through entitybunnyfinder_client

## Coding Patterns

- `useHistoriesManager` is the primary hook -- it orchestrates entitybunnyfinder_client hooks + Zustand store into a single unified interface for UI layers
...
- This is a published npm package (`@sudobility/entitybunnyfinder_lib`) -- coordinate breaking changes with entitybunnyfinder_app and entityentitybunnyfinder_app_rn
